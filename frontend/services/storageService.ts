...
// Cache duration in milliseconds (e.g., 20 seconds)
const CACHE_DURATION = 1 * 20 * 1000;
  ...
  getLogs: async (): Promise<RoutineLog[]> => {
    ...
    try {
      ...
      const logs: RoutineLog[] = logsData.map((log: any) => ({
        ...
        moodRating:
          typeof log.felt_today === "string"
            ? moodMapReverse[log.felt_today] || 6
            : log.felt_today || 6,
      }));
  ...
  getProfile: async (): Promise<UserProfile> => {
    ...
    try {
      ...
      const profile: UserProfile = {
        ...
        profilePicture:
          localStorage.getItem("studentflow_user_pfp") || data.pfp_url || "",
        goals:
          typeof data.academic_goals === "string"
            ? JSON.parse(data.academic_goals)
            : data.academic_goals || [],
        examDates:
          typeof data.upcoming_exams === "string"
            ? JSON.parse(data.upcoming_exams)
            : data.upcoming_exams || [],
        settings: {
